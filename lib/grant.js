...
function usePasswordGrant (done) {
  ...
  var self = this;
  return this.model.getUser(uname, pword, function (err, user, userExists) {
    if (err) return done(error('server_error', false, err));
    if (userExists == 0) {
      return done(error('invalid_grant', 'User credentials are invalid'));
    }
    else if (userExists == 1) {
      return done(error('invalid_password', 'User credentials are invalid'));
    }
...
function saveRefreshToken (done) {
  ...
  console.log("clientId: "+JSON.stringify(this.client.clientId));
